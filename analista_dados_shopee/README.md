...
> ⚠️ **Use sempre a flag `--env-file CHAVES_DADOS.env`** nos comandos do Docker Compose deste projeto — sem ela, variáveis como `DB_PORT` e `TZ` não chegam ao `docker-compose.yml` corretamente. Veja o motivo no item 1 de [Pontos de Atenção](#️-pontos-de-atenção--erros-conhecidos).
...
pip install tabulate   # necessário para o Chat de Dados formatar tabelas — veja item 4 abaixo
```
...
> ⚠️ Se aparecer **"Falha no Boot da IA (Timeout)"**, veja o item 2 e 3 de [Pontos de Atenção](#️-pontos-de-atenção--erros-conhecidos) — há dois detalhes para revisar antes do `llm.py` subir corretamente.
